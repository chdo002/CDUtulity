...
- (void)aat_setImageLoadOperation:(nullable id)operation forKey:(nullable NSString *)key {
    if (key) {
        [self aat_cancelImageLoadOperationWithKey:key];
        if (operation) {
...
- (void)aat_cancelImageLoadOperationWithKey:(nullable NSString *)key {
    ...
    if (operations) {
        if ([operations isKindOfClass:[NSArray class]]) {
            for (id <AATWebImageOperation> operation in operations) {
                if (operation) {
                    [operation cancel];
                }
            }
        } else if ([operations conformsToProtocol:@protocol(AATWebImageOperation)]){
            [(id<AATWebImageOperation>) operations cancel];
        }
